...
class BeamSearchDecoder(object):
  ...
  def __init__(self, model, batcher, vocab):
    ...
    self._output_article = False  # Set to false to output original articles.
    if FLAGS.single_pass:
      ...
      if self._output_article:
        self._article_dir = os.path.join(self._decode_dir, "article")
        if not os.path.exists(self._article_dir): os.mkdir(self._article_dir)
  ...
  def decode(self):
    ...
    while True:
      ...
      if batch is None or counter > FLAGS.decode_limit: # finished decoding dataset in single_pass mode
      ...
      # Use the below to resume (needs disable dir-exists guard above)
      if FLAGS.single_pass:
        decoded_filename = os.path.join(self._rouge_dec_dir, "%06d_decoded.txt" % counter)
      ...
      total_len += len(decoded_words)
      decoded += 1
      print ("  avg length: %s" % (total_len / decoded))

      if FLAGS.single_pass:
        self.write_for_rouge(original_abstract_sents, decoded_words, counter, article=(original_article if self._output_article else None)) # write ref summary and decoded summary to file, to eval with pyrouge later
  ...
  def write_for_rouge(self, reference_sents, decoded_words, ex_index, article=None):
    ...
    if article is not None:
      article = [make_html_safe(article)]
    ...
    if article is not None:
      article_file = os.path.join(self._article_dir, "%06d_article.txt" % ex_index)
