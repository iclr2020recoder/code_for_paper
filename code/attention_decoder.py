...
def attention_decoder(decoder_inputs, initial_state, encoder_states, enc_padding_mask, cell,
  ...
  Args:
    ...
    initial_state_attention:
      Note that this attention decoder passes each decoder input through a linear layer with the previous step's context vector to get a 
      modified version of the input. If initial_state_attention is False, on the first decoder step the "previous context vector" 
      is just a zero vector. If initial_state_attention is True, we use initial_state to (re)calculate the previous step's context vector.
      We set this to False for train/eval mode (because we call attention_decoder once for all decoder steps) and True for decode mode
  ...
  with variable_scope.variable_scope("attention_decoder") as scope:
    ...
    def attention(decoder_state, coverage=None):
      ...
      with variable_scope.variable_scope("Attention"):
        ...
        if use_coverage and coverage is not None: # non-first step of coverage
          ...
          # encoder_features has shape (batch_size, <enc input length>, 1, attention-size)
          # decoder_features has shape (batch_size, 1, 1, attention-size)
          # The shapes don't match but TF automatically tiles decoder_features.
          e = math_ops.reduce_sum(v * math_ops.tanh(encoder_features + decoder_features), [2, 3]) # e has shape (batch_size, <enc input length>)
        ...
        # Context vector has shape (batch_size, attention_size)
    ...
    if input_size.value is None:
      raise ValueError("Could not infer input size from input: %s" % decoder_inputs[0].name)

    for i in range(steps):
      tf.logging.info("Adding attention_decoder timestep %i of %i", i, steps)
      if i > 0:
      ...
      with variable_scope.variable_scope("AttnOutputProjection"):
        output = linear([cell_output] + [context_vector], hidden_dim, True)        
    ...
    return outputs, state, states, attn_dists, p_gens, coverage
