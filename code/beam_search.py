...
def run_beam_search(sess, model, vocab, batch):
  ...
  while steps < FLAGS.max_dec_steps and len(results) < FLAGS.beam_size:    
    ...
    USE_BEAM = steps == 0 and FLAGS.beam_decode

    # Run one step of the decoder to get the new info
    (topk_ids, topk_log_probs, new_states, attn_dists, p_gens, new_coverage, first_ids) = model.decode_onestep(sess=sess,
                        batch=batch,
                        latest_tokens=latest_tokens,
                        enc_states=enc_states,
                        dec_init_states=states,
                        prev_coverage=prev_coverage,
                        return_first=USE_BEAM)

    if USE_BEAM:
      if FLAGS.single_pass and FLAGS.beam_decode:
        print ('Using beam_decode: %s' % data.outputids2words(first_ids[0], vocab, batch.art_oovs[0]))
        hyp = hyps[0]
        for id in first_ids[0]: 
          hyp = hyp.extend(
            token = id,
            log_prob=1.0,
            state=new_states[0],
            ...
            p_gen=p_gens[0],
            coverage=new_coverage[0])
          #if id == vocab.word2id(data.STOP_DECODING):
          #  break
        return hyp
      elif not FLAGS.single_pass:
        print ('Beam decoder: %s' % data.outputids2words(first_ids[0], vocab, batch.art_oovs[0]))
    ...
    if DEB_OUTPUT: print ("Starting new timestep %s-------------------" % steps)    
    for i in range(num_orig_hyps):
      ...
      if DEB_OUTPUT: print ('  For past tokens = %s' % data.outputids2words(h.tokens, vocab, batch.art_oovs[0]))            
      for j in range(FLAGS.beam_size * 2):  # for each of the top 2*beam_size hyps:
        ...
        if DEB_OUTPUT: print ('   got candidate %s' % data.outputids2words(new_hyp.tokens[-1:], vocab, batch.art_oovs[0]))        
